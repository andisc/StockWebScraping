...
def main(id_control):
    try:
        url = 'http://investors.fivestarseniorliving.com/news/press-releases/default.aspx' 
        ...
        #print(soup)
        articles_panel = soup.find('div', attrs={'class':'module_container module_container--content'})
        articles = articles_panel.findAll('div', attrs={'class':'module_item'})
        
        # get first article
        FIRST_ARTICLE = articles[0]

        article_date = FIRST_ARTICLE.find('div', attrs={'class':'module_date-time'})
        article_desc = FIRST_ARTICLE.find('div', attrs={'class':'module_headline'})
