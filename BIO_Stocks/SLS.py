...
def main(id_control):
    try:
        url = 'https://www.sellaslifesciences.com/investors/news/default.aspx' 
        ...
        #print(soup)
        articles_panel = soup.find('div', attrs={'class':'items newsbody'})
        articles = articles_panel.findAll('div', attrs={'class':'items-list'})
        
        # get first article
        FIRST_ARTICLE = articles[0]

        article_date = FIRST_ARTICLE.find('p', attrs={'class':'date'})
        article_desc = FIRST_ARTICLE.find('p', attrs={'class':'title'})
