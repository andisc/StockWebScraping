...
def main(id_control):
    try:
        url = 'https://igmbio.com/news/press-releases/' 
        ...
        #print(soup)
        articles_panel = soup.find('div', attrs={'class':'fl-post-feed'})
        articles = articles_panel.findAll('div', attrs={'class':'fl-post-feed-post'})
        
        # get first article
        FIRST_ARTICLE = articles[0]

        article_date = FIRST_ARTICLE.find('span', attrs={'class':'fl-post-feed-date'})
        article_desc = FIRST_ARTICLE.find('h2', attrs={'class':'fl-post-feed-title'})
