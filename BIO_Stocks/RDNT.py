...
def main(id_control):
    try:
        ...
        #print(soup)
        articles_panel = soup.find('div', attrs={'class':'newslist list'})
        articles = articles_panel.findAll('div', attrs={'class':'newsitems'})
        
        # get first article
        FIRST_ARTICLE = articles[0]
        
        article_date_all = FIRST_ARTICLE.findAll('div', attrs={'class':'pubdate'})
        article_date = article_date_all[0]
        article_desc_all = FIRST_ARTICLE.findAll('div', attrs={'class':'title'})
        article_desc = article_desc_all[0]
        article_url_all = FIRST_ARTICLE.findAll('a')
        article_url = article_url_all[0]
