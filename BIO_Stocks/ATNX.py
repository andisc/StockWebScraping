...
def main(id_control):
    try:
        url = 'https://ir.athenex.com/press-releases/' 
        ...
        #print(soup)
        articles = soup.findAll('div', attrs={'class':'ccbnResponsiveRowSmall ccbnStackCell'})
        
        # get first article
        FIRST_ARTICLE = articles[0]

        article_date = FIRST_ARTICLE.find('div', attrs={'class':'ccbnNewsDate'})
        article_desc = FIRST_ARTICLE.find('span', attrs={'class':'ccbnSubTtl'})
        article_url = FIRST_ARTICLE.find('div', attrs={'class':'ccbnMore'})
