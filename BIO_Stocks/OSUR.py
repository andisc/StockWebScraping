...
def main(id_control):
    try:
        url = 'https://www.orasure.com/press.html' 
        ...
        #print(soup)
        articles_panel = soup.find('div', attrs={'id':'press_releases'})
        articles = articles_panel.findAll('div', attrs={'class':'row row-space'})
        
        # get first article
        FIRST_ARTICLE = articles[0]

        article_date = FIRST_ARTICLE.find('div', attrs={'class':'press-release-date'})
        article_desc = FIRST_ARTICLE.find('div', attrs={'class':'press-release-head'})
