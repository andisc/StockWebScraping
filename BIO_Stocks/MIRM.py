...
def main(id_control):
    try:
        url = 'https://ir.mirumpharma.com/news-releases/' 
        ...
        #print(soup)
        articles = soup.findAll('section', attrs={'class':'av_toggle_section'})
        
        # get first article
        FIRST_ARTICLE = articles[0]

        article_date = FIRST_ARTICLE.find('div', attrs={'class':'nir-widget--field nir-widget--news--date-time'})
        article_desc = FIRST_ARTICLE.find('p', attrs={'class':'toggler'})
