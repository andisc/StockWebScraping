...
def main(id_control):
    try:
        url = 'http://www.iradimed.com/en-us/about-iradimed/news' 
        ...
        #print(soup)
        articles_panel = soup.find('div', attrs={'class':'summary-item-list sqs-gallery'})
        articles = articles_panel.findAll('div', attrs={'class':'summary-item'})
        
        # get first article
        FIRST_ARTICLE = articles[0]

        article_date = FIRST_ARTICLE.find('time', attrs={'class':'summary-metadata-item summary-metadata-item--date'})
        article_desc = FIRST_ARTICLE.find('div', attrs={'class':'summary-title'})
