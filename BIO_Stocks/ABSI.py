...
def main(id_control):
    try:
        url = 'https://investors.absci.com/news-and-events/news-releases' 
        req = Request(
            url=url, 
        ...
        articles = articles_panel.findAll('article', attrs={'role':'article'})
        
        # get first article
        FIRST_ARTICLE = articles[0]

        article_date = FIRST_ARTICLE.find('span', attrs={'class':'press-date'})
        article_year = FIRST_ARTICLE.find('span', attrs={'class':'press-year'})
        article_desc = FIRST_ARTICLE.find('div', attrs={'class':'nir-widget--field nir-widget--news--headline'})
        
        v_article_date = article_date.text.lstrip().rstrip() + " " + article_year.text.lstrip().rstrip() 
