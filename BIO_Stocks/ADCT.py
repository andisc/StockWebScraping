...
def main(id_control):
    try:
        url = 'https://ir.adctherapeutics.com/press-releases/default.aspx' 
        req = Request(
            url=url, 
        ...
        articles_panel = soup.find('div', attrs={'class':'evergreen-container evergreen-container-content evergreen-news-content evergreen-news-content-list'})
        articles = articles_panel.findAll('div', attrs={'class':'evergreen-item evergreen-news-item evergreen-item-container'})
        
        # get first article
        FIRST_ARTICLE = articles[0]

        article_date = FIRST_ARTICLE.find('div', attrs={'class':'evergreen-item-date-time evergreen-news-date'})
        article_desc = FIRST_ARTICLE.find('div', attrs={'class':'evergreen-news-headline'})
