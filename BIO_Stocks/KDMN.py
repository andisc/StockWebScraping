...
def main(id_control):
    try:
        url = 'https://investors.kadmon.com/press-releases/' 
        ...
        #print(soup)
        articles = soup.findAll('section', attrs={'class':'press-release'})
        
        # get first article
        FIRST_ARTICLE = articles[0]

        article_date = FIRST_ARTICLE.find('p', attrs={'class':'post-date'})
        article_desc = FIRST_ARTICLE.find('h2', attrs={'class':'post-title'})
        article_url = FIRST_ARTICLE.find('div', attrs={'itemprop':'description'})
