...
def main(id_control):
    try:
        url = 'https://investor.synlogictx.com/news-and-events/news-releases' 
        ...
        articles = articles_panel.findAll('div', attrs={'class':'news_grid_column'})
        
        # get first article
        FIRST_ARTICLE = articles[0]

        article_date = FIRST_ARTICLE.find('div', attrs={'class':'nir-widget--field nir-widget--news--date-time'})
        article_desc = FIRST_ARTICLE.find('div', attrs={'class':'news_grid_title'})
        article_url = FIRST_ARTICLE.find('div', attrs={'class':'nir-widget--field nir-widget--news--supporting-materials'})
