...
def main(id_control):
    try:
        url = 'https://investors.frequencytx.com/news-releases/' 
        ...
        #print(soup)
        articles = soup.findAll('li', attrs={'class':'c-news__item'})
        
        # get first article
        FIRST_ARTICLE = articles[0]

        article_date_day = FIRST_ARTICLE.find('span', attrs={'class':'c-news__day'})
        article_date_month = FIRST_ARTICLE.find('span', attrs={'class':'c-news__month'})
        article_date_year = datetime.now().year
        article_date = (article_date_month.text + ' ' + article_date_day.text + ', ' + str(article_date_year))
        article_desc = FIRST_ARTICLE.find('div', attrs={'class':'c-news__body'})
