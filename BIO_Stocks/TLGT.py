...
def main(id_control):
    try:
        url = 'http://investors.teligent.com/news-and-events/news-releases' 
        ...
        #print(soup)
        articles_panel = soup.find('div', attrs={'class':'item-list'})
        articles = articles_panel.findAll('li', attrs={'class':'has-news-supp'})
        
        # get first article
        FIRST_ARTICLE = articles[0]

        article_date = FIRST_ARTICLE.find('div', attrs={'class':'views-field views-field-field-nir-news-date'})
        article_desc_panel = FIRST_ARTICLE.find('div', attrs={'class':'views-field views-field-field-nir-news-title'})
        all_a = article_desc_panel.findAll('a')
