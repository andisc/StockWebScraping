...
def main(id_control):
    try:
        url = 'https://investors.otonomy.com/news-releases' 
        ...
        #print(soup)
        articles_panel = soup.find('ul', attrs={'class':'listing__list'})
        articles = articles_panel.findAll('li', attrs={'class':'listing__list__item'})
        
        # get first article
        FIRST_ARTICLE = articles[0]

        article_date = FIRST_ARTICLE.find('span', attrs={'class':'listing__list__item__date'})
        article_desc = FIRST_ARTICLE.find('span', attrs={'class':'listing__list__item__link listing__list__item__link--filter'})
