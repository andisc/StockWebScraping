...
def main(id_control):
    try:
        url = 'https://www.legendbiotech.com/press-releases.php' 
        ...
        #print(soup)
        articles_panel = soup.find('div', attrs={'class':'year-group flex-column active'})
        articles = articles_panel.findAll('div', attrs={'class':'text-row-2 flex align-items-center'})
        
        # get first article
        FIRST_ARTICLE = articles[0]

        article_date = FIRST_ARTICLE.find('span', attrs={'class':'date'})
        article_desc = FIRST_ARTICLE.find('div', attrs={'class':'text-column padding-left-10 description-container'})
