...
def main(id_control):
    try:
        url = 'https://abvcpharma.com/?page_id=17318' 
        req = Request(
            url=url, 
        ...
        articles_panel = soup.find('div', attrs={'class':'grve-post-leader'})
        articles = articles_panel.findAll('article', attrs={'class':'grve-blog-item post-17697 post type-post status-publish format-standard hentry category-adhd category-press-release'})
        
        # get first article
        FIRST_ARTICLE = articles[0]

        article_date = FIRST_ARTICLE.find('div', attrs={'class':'nir-widget--field nir-widget--news--date-time'})
        article_desc = FIRST_ARTICLE.find('li', attrs={'class':'grve-post-date'})
