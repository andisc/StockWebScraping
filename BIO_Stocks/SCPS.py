...
def main(id_control):
    try:
        url = 'https://scopusbiopharma.com/news-and-events/press-releases/' 
        ...
        #print(soup)
        articles = soup.findAll('article', attrs={'class':'elementor-post elementor-grid-item post-996 post type-post status-publish format-standard hentry category-press-release'})
        
        # get first article
        FIRST_ARTICLE = articles[0]

        article_date = FIRST_ARTICLE.find('div', attrs={'class':'elementor-post__meta-data'})
        article_desc = FIRST_ARTICLE.find('h3', attrs={'class':'elementor-post__title'})
