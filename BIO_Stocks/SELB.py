...
def main(id_control):
    try:
        url = 'https://ir.selectabio.com/press-releases' 
        ...
        articles = articles_panel.findAll('div', attrs={'class':'row press-list overlay-fade-in revealOnScroll-fadeInUp'})
        
        # get first article
        FIRST_ARTICLE = articles[0]

        article_date_day = FIRST_ARTICLE.find('span', attrs={'class':'press-date'}).text.lstrip().rstrip()
        article_date_month = FIRST_ARTICLE.find('span', attrs={'class':'press-year'}).text.lstrip().rstrip()
        article_date = (article_date_day + ' ' + article_date_month + ' ' + str(datetime.now().year))
        article_desc = FIRST_ARTICLE.find('h5')
