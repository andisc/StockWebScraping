...
def main(id_control):
    try:
        url = 'https://www.gbs.com/en/pressreleases' 
        ...
        #print(soup)
        articles_panel = soup.find('div', attrs={'class':'mod_newsarchive headline news block'})
        articles = articles_panel.findAll('div', attrs={'class':'layout_latest block first even'})
        
        # get first article
        FIRST_ARTICLE = articles[0]

        article_date_header = FIRST_ARTICLE.find('div', attrs={'class':'header'})
        article_date_time = article_date_header.find('time').get('datetime')
        article_date = (str(article_date_time)[0:10])
