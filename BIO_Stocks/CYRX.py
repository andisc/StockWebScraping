...
def main(id_control):
    ...
    try:
        url = 'http://ir.cryoport.com/news-releases/' 
        ...
        table = soup.find('table', attrs={'class':'prReports reportTable'})
        #print(table)
        table_body = table.find('tbody')
        rows = table_body.find_all('tr')

        
        FIRST_ROW_columns_header = rows[0].find_all('th')
        FIRST_ROW_columns = rows[0].find_all('td')
        v_article_date = FIRST_ROW_columns_header[0].text.replace("DATE", "").lstrip().rstrip()
        article_desc = FIRST_ROW_columns[1].find('a')
