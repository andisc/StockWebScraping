...
def main(id_control):
    ...
    try:
        url = 'https://ir.fatetherapeutics.com/press-releases' 
        ...
        table = soup.find('table', attrs={'class':'nirtable views-table views-view-table cols-3 collapse-table'})
        #print(table)
        ...
        v_article_date_month = FIRST_ROW_columns[0].find('span', attrs={'class':'event_mo'}).text.lstrip().rstrip()
        v_article_date_day = FIRST_ROW_columns[0].find('span', attrs={'class':'event_day'}).text.lstrip().rstrip()
        v_article_date_year = FIRST_ROW_columns[0].find('span', attrs={'class':'event_yr'}).text.lstrip().rstrip()
        v_article_date = (v_article_date_month + ' ' + v_article_date_day + ', ' + v_article_date_year)
        article_desc = FIRST_ROW_columns[1].find('h3', attrs={'class':'press_title ndq_item_title'})
