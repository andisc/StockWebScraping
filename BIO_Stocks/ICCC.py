...
def main(id_control):
    try:
        url = 'https://immucell.com/investors/news/' 
        ...
        #print(soup)
        articles_panel = soup.find('div', attrs={'class':'wpb_text_column wpb_content_element'})
        articles = articles_panel.findAll('p')
        
        
        # get first article
        FIRST_ARTICLE = articles[1]

        article_date = FIRST_ARTICLE.text[0:8]
        article_desc = FIRST_ARTICLE.text.replace(article_date, '')
        ...
        if (istoday == True):
            ...
            v_description = article_desc.lstrip().rstrip()
            now = datetime.now()
