...
def main(id_control):
    try:
        url = 'https://investors.dyne-tx.com/news-and-events/press-releases/' 
        ...
        #print(soup)
        articles_panel = soup.find('div', attrs={'class':'row press-releases-list'})
        articles = articles_panel.findAll('div', attrs={'class':'card-body'})
        
        # get first article
        FIRST_ARTICLE = articles[0]

        article_date = FIRST_ARTICLE.find('h4', attrs={'class':'card-title text-nowrap'})
        article_desc = FIRST_ARTICLE.find('div', attrs={'class':'card-text'})
