...
def main(id_control):
    try:
        url = 'https://investors.solidbio.com/news-and-events/press-releases' 
        ...
        #print(soup)
        articles_panel = soup.find('ul', attrs={'class':'c-news'})
        articles = articles_panel.findAll('li', attrs={'role':'article'})
        
        # get first article
        FIRST_ARTICLE = articles[0]

        article_date_day = FIRST_ARTICLE.find('span', attrs={'class':'c-news__day'}).text.lstrip().rstrip()
        article_date_month = FIRST_ARTICLE.find('span', attrs={'class':'c-news__month'}).text.lstrip().rstrip()
        article_date = (article_date_day + ' ' + article_date_month + ' ' + str(datetime.now().year))
        article_desc = FIRST_ARTICLE.find('h3', attrs={'class':'c-news__title'})
