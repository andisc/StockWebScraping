...
def main(id_control):
    try:
        url = 'http://investors.secondsight.com/press-releases' 
        ...
        #print(soup)
        articles_panel = soup.find('div', attrs={'class':'nir-widget--list download-section-inner-list'})
        articles = articles_panel.findAll('div', attrs={'class':'download-item'})
        
        # get first article
        FIRST_ARTICLE = articles[0]

        article_date = FIRST_ARTICLE.find('div', attrs={'class':'download-item-middle dli-news-date'})
        article_desc = FIRST_ARTICLE.find('div', attrs={'class':'download-item-left dli-news-title'})
