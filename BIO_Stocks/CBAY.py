...
def main(id_control):
    try:
        url = 'https://ir.cymabay.com/press-releases' 
        ...
        #print(soup)
        articles = soup.findAll('div', attrs={'class':'blog_post'})
        
        # get first article
        FIRST_ARTICLE = articles[0]

        article_date_day = FIRST_ARTICLE.find('strong')
        article_date_month = FIRST_ARTICLE.find('i')
        article_date_year = datetime.today().year
        article_date = (article_date_month.text + ' ' + article_date_day.text + ', '+ str(article_date_year))

        article_desc = FIRST_ARTICLE.find('h3')
