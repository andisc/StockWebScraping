...
def main(id_control):
    try:
        url = 'https://ir.tffpharma.com/press-releases/' 
        ...
        #print(soup)
        articles_panel = soup.find('section', attrs={'class':'ir-press-releases'})
        articles = articles_panel.findAll('a')
        
        # get first article
        FIRST_ARTICLE = articles[1]

        article_date = FIRST_ARTICLE.find('p', attrs={'class':'date'})
        article_desc = FIRST_ARTICLE.find('p', attrs={'class':'pr-title'})
