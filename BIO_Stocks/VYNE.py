...
def main(id_control):
    try:
        url = 'https://vynetherapeutics.com/category/press-releases/' 
        ...
        #print(soup)
        articles = soup.findAll('article', attrs={'class':'preview preview-post post-69397 post type-post status-publish format-standard hentry category-press-releases'})
        
        # get first article
        FIRST_ARTICLE = articles[0]

        article_date = FIRST_ARTICLE.find('p')
        article_desc = FIRST_ARTICLE.find('h2', attrs={'class':'preview-title heading-size-3'})
