...
def main(id_control):
    ...
    try:
        ...
        v_article_date = FIRST_ROW_columns[0].find('div', attrs={'class':'nir-widget-field nir-widget--news--date-time date'}).text.lstrip().rstrip()
        #v_article_date = FIRST_ROW_columns[0].text.lstrip().rstrip()
        article_desc = FIRST_ROW_columns[0].find('div', attrs={'class':'nir-widget-field nir-widget--news--headline description'})
