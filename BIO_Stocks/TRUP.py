...
def main(id_control):
    try:
        url = 'https://investors.trupanion.com/news-and-events/press-releases/default.aspx' 
        ...
        #print(soup)
        articles_panel = soup.find('div', attrs={'class':'news-list'})
        articles = articles_panel.findAll('div', attrs={'class':'ModuleItemRow'})
        
        # get first article
        FIRST_ARTICLE = articles[0]

        article_date = FIRST_ARTICLE.find('span', attrs={'class':'ModuleDate'})
        article_desc = FIRST_ARTICLE.find('span', attrs={'class':'ModuleHeadline'})
