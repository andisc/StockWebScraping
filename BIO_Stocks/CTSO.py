...
def main(id_control):
    try:
        url = 'https://cytosorbents.com/news1/press-releases/' 
        ...
        #print(soup)
        articles_panel = soup.find('ul', attrs={'class':'press-release-post'})
        articles = articles_panel.findAll('li')
        
        # get first article
        FIRST_ARTICLE = articles[0]

        article_date = FIRST_ARTICLE
        article_desc = FIRST_ARTICLE.find('a')
        v_article_date = article_date.text.replace(article_desc.text, '').lstrip().rstrip()
