...
def main(id_control):
    try:
        url = 'https://www.soctelemed.com/press-room/' 
        ...
        #print(soup)
        articles_panel = soup.find('div', attrs={'class':'container-pressrelease'})
        articles = articles_panel.findAll('div', attrs={'class':'single-list'})
        
        # get first article
        FIRST_ARTICLE = articles[0]

        article_date = FIRST_ARTICLE.find('div', attrs={'class':'date'})
        article_desc = FIRST_ARTICLE.find('h1')
        article_url = FIRST_ARTICLE.find('div', attrs={'class':'press-release-thumb-link-container'})
