...
def main(id_control):
    ...
    try:
        url = 'https://atrion.irpass.com/News' 
        ...
        table = soup.find('table', attrs={'class':'b2iNewsItem1'})

        rows = table.find_all('tr')

        
        FIRST_ROW_columns = rows[0].find_all('td')
        v_article_date = FIRST_ROW_columns[0].find('span', attrs={'class':'b2iNewsItemDate'}).text.lstrip().rstrip()
        article_desc = FIRST_ROW_columns[0].find('span', attrs={'class':'b2iNewsItemHeadline'})
