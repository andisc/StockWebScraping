...
def main(id_control):
    try:
        url = 'https://investors.iteostherapeutics.com/news-and-events/press-releases/' 
        ...
        #print(soup)
        articles_panel = soup.find('ul', attrs={'class':'news__list news__list--all'})
        articles = articles_panel.findAll('li', attrs={'class':'news__elt'})
        
        # get first article
        FIRST_ARTICLE = articles[0]

        article_date = FIRST_ARTICLE.find('time')
        article_desc = FIRST_ARTICLE.find('h3', attrs={'class':'news__title'})
