...
def main(id_control):
    try:
        url = 'https://www.hudsongroup.com/media/press-releases' 
        ...
        #print(soup)
        articles_panel = soup.find('ul', attrs={'class':'press-release-list list'})
        articles = articles_panel.findAll('li', attrs={'class':'press-release-item'})
        
        # get first article
        FIRST_ARTICLE = articles[0]
        ARTICLE = FIRST_ARTICLE.find('div', attrs={'class':'teaser--press-release'})

        article_date = ARTICLE.find('time')
        article_desc = ARTICLE.find('h3')
