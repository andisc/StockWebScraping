...
def main(id_control):
    try:
        url = 'https://ir.abivax.com/news-and-events/news-releases' 
        req = Request(
            url=url, 
        ...
        articles_panel = soup.find('div', attrs={'class':'nir-widget--content'})
        articles = articles_panel.findAll('article', attrs={'class':'clearfix node node--nir-news--nir-widget-list node--type-nir-news node--view-mode-nir-widget-list node--promoted nir-widget-article llf-border'})
