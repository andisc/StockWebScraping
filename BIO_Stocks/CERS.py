...
def main(id_control):
    try:
        url = 'https://ir.cerus.com/news-and-events/press-releases/default.aspx' 
        ...
        #print(soup)
        articles = soup.findAll('div', attrs={'class':'module_item grid grid--no-gutter grid--flex grid--flex_align-middle module_item--date-left'})
        
        # get first article
        FIRST_ARTICLE = articles[0]
        print(FIRST_ARTICLE)

        article_date = FIRST_ARTICLE.find('div', attrs={'class':'byline'})
        article_desc = FIRST_ARTICLE.find('div', attrs={'class':'article'})
