...
def main(id_control):
    try:
        url = 'http://investors.tenaxthera.com/TENX/press_releases?template=pr_date_above&per_page=15&years_pagination=1' 
        ...
        #print(soup)
        articles_panel = soup.find('div', attrs={'class':'press_releases'})
        articles = articles_panel.findAll('div', attrs={'class':'pr_item'})
        
        # get first article
        FIRST_ARTICLE = articles[0]

        article_date = FIRST_ARTICLE.find('div', attrs={'class':'pr_date'})
        article_desc = FIRST_ARTICLE.find('div', attrs={'class':'pr_headline'})
