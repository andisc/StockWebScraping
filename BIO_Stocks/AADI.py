...
def main(id_control):
    try:
        url = 'https://aadibio.com/latest-news/' 
        req = Request(
            url=url, 
        ...
        articles_panel = soup.find('div', attrs={'class':'ui--blog ui--blog-wrapper ui--pass'})
        articles = articles_panel.findAll('div', attrs={'class':'ui--blog-content-wrapper'})
        
        # get first article
        FIRST_ARTICLE = articles[0]

        article_date = FIRST_ARTICLE.find('span', attrs={'class':'datetime'})
        article_desc = FIRST_ARTICLE.find('div', attrs={'class':'ui--blog-content'})
