...
def main(id_control):
    try:
        url = 'https://catasys.com/press-releases.html' 
        ...
        #print(soup)
        articles_panel = soup.find('div', attrs={'class':'qmod-news-list-group'})
        articles = articles_panel.findAll('ul', attrs={'class':'qmod-news-list'})
        
        # get first article
        FIRST_ARTICLE = articles[0]

        article_date = FIRST_ARTICLE.find('div', attrs={'class':'qmod-news-datetime'})
        article_desc = FIRST_ARTICLE.find('a', attrs={'class':'qmod-headline'})
