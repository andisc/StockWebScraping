...
def main(id_control):
    try:
        url = 'https://investors.kaleido.com/press-releases/' 
        ...
        #print(soup)
        articles = soup.findAll('article', attrs={'class':'post-973 post type-post status-publish format-standard hentry category-august-2018'})
        
        # get first article
        FIRST_ARTICLE = articles[0]

        article_date_day = FIRST_ARTICLE.find('span', attrs={'class':'date'}).text.lstrip().rstrip() 
        article_date_month = FIRST_ARTICLE.find('span', attrs={'class':'month'}).text.lstrip().rstrip() 
        article_date_year = FIRST_ARTICLE.find('span', attrs={'class':'year'}).text.lstrip().rstrip() 
        article_date = (article_date_day + ' ' + article_date_month + ' ' + article_date_year)
        article_desc = FIRST_ARTICLE.find('h2', attrs={'class':'post-title'})
