...
def main(id_control):
    try:
        url = 'https://ir.genocea.com/press-releases' 
        ...
        #print(soup)
        articles = soup.findAll('div', attrs={'role':'article'})
        
        # get first article
        FIRST_ARTICLE = articles[0]

        article_date = FIRST_ARTICLE.find('div', attrs={'class':'posts--date color-white font-weight-bold'})
        article_desc = FIRST_ARTICLE.find('div', attrs={'class':'posts--title color-black'})
        article_url = FIRST_ARTICLE.find('div', attrs={'class':'posts--button'})
