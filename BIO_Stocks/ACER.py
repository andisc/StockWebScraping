...
def main(id_control):
    try:
        url = 'https://www.acertx.com/investor-relations/press-releases/' 
        ...
        #print(soup)
        articles = soup.findAll('div', attrs={'class':'jiu revealer'})
        
        # get first article
        FIRST_ARTICLE = articles[0]

        article_date = FIRST_ARTICLE.find('div', attrs={'class':'dati'})
        article_desc = FIRST_ARTICLE.find('h2', attrs={'class':'titkelrpress'})
        article_url = FIRST_ARTICLE.find('span', attrs={'class':'remk'})
